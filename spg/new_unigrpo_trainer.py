...
try:
    ...
    # Fallback for TRL versions without profiling utilities
    from contextlib import contextmanager

    def profiling_decorator(*_args, **_kwargs):
        def _wrap(fn):
            return fn
        return _wrap
    ...
    @contextmanager
    def profiling_context(*_args, **_kwargs):
        yield
...
try:
    ...
    def print_prompt_completions_sample(*_args, **_kwargs):
        # Fallback for TRL versions without this helper
        return
import wandb
...
class newUniGRPOTrainer(GRPOTrainer):
    """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if eps is None:
            eps = getattr(self.args, "epsilon", None)
        if eps is None:
            eps = getattr(self.args, "clip_range", 0.2)  # PPO/GRPO常用默认
    ...
    def forward_process(self, batch, prompt_index, mask_id, seed=None, completion_mask=None, mask_ratio=None):
        """
        ...
        if forward_type == "random":
            ...
            for i in range(b):
                ...
                if mask_ratio is None:
                    ...
                    # yy- UniGRPO: fixed strength (mask_ratio = t_n)
                    L_i = int(completion_length[i].item())
                    fixed = int(round(L_i * float(mask_ratio)))
                    fixed = max(start_mask_num, min(fixed, end_mask_num))
                    mask_num = torch.full((1, num_t), fixed, device=device, dtype=torch.long)
    ...
    def _get_per_token_logps(self, model, input_ids, logits_to_keep, mask_seeds, completion_mask, mask_ratio=None):
        """
        ...
        if mask_ratio is not None:
            assert self.args.num_t == 1, "UniGRPO num_t=1"
        ...
        for iter_idx, mask_seed in enumerate(mask_seeds):
            ...
            if mask_ratio is None:
                avg_logps = -nll_per_token.mean(dim=1)      # [B,L]
            else:
                # yy: UniGRPO 固定t_n的oi logp
                avg_logps = -nll_per_token[:, 0, :]         # [B,L]
    ...
    def _prepare_inputs(
        ...
        # ---- hard guard: make sure buffer exists ----
        gas = int(getattr(self.args, "gradient_accumulation_steps", 1))
        if getattr(self, "_buffered_inputs", None) is None:
            self._buffered_inputs = [None] * gas
        if not hasattr(self, "_step") or self._step is None:
            self._step = 0
        mode = "eval" if self.control.should_evaluate else "train"
        if mode == "train":
    ...
    def _generate_and_score_completions(
        ...
        # yy- 均匀采样t1……tμ-1
        # diffusion 总步数是 T (self.args.diffusion_steps)
        ...
        # yy- 对于每个tn，提前保存old logps和ref logps
        with torch.no_grad():
            # 逐个 t_n 计算 old logps，得到 [mu,B,L]
            old_list = []
            for n in range(self.num_iterations):
                old_n = self._get_per_token_logps(
                    self.model,
                    prompt_completion_ids.unsqueeze(0),  # [1,B,P+L]
                    logits_to_keep,
                    [mask_seeds[n]],
                    completion_mask,
                    mask_ratio=float(mask_ratios[n].item()),  # NEW
                )  # -> [1,B,L]
                old_list.append(old_n)
            old_per_token_logps = torch.cat(old_list, dim=0)  # [mu,B,L]

            # ref logps 同理（如果 beta!=0）
            if self.beta == 0.0:
                ref_per_token_logps = None
            else:
                ref_list = []
                with self.accelerator.unwrap_model(self.model).disable_adapter():
                    for n in range(self.num_iterations):
                        ref_n = self._get_per_token_logps(
                            self.model,
                        ...
                        ref_list.append(ref_n)
                ref_per_token_logps = torch.cat(ref_list, dim=0)  # [mu,B,L]
        ...
        return {
            ...
            "mask_ratios": mask_ratios  # yy-add
        }