...
class SWIFTTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions_for_semi_and_early(
        ...
        # =========================================================================
        # [新增] 在此处调用方差测量
        # 必须传入 generation_mask 和 early_rollout_token_index
        # =========================================================================
        if getattr(self.args, "log_elbo_variance", True) and (self.state.global_step % self.args.logging_steps == 0):
            # 获取文本用于日志
            completions_text = self.processing_class.batch_decode(completion_ids, skip_special_tokens=True)
            self.measure_elbo_variance_semi_offline(
    ...
    def measure_elbo_variance_semi_offline(
            self, 
            prompt_completion_ids, 
            logits_to_keep, 
            completion_mask, 
            ...
            """
            在 Semi-Offline 设定下测量 ELBO 方差。
            关键点：必须传入 generation_mask，确保 K 次重复计算都遵守相同的 token 固定约束。
            """
            # 仅主进程执行
            ...
            # 参数配置
            K = getattr(self.args, "elbo_variance_k", 3) # 默认重复 16 次
            limit = getattr(self.args, "variance_sample_limit", 12) # 仅分析 Batch 前 2 个样本
            
            # 截取子集
            sample_ids = prompt_completion_ids[:limit]
            sample_comp_mask = completion_mask[:limit]
            sample_gen_mask = generation_mask[:limit] if generation_mask is not None else None
            sample_early_idx = early_rollout_token_index[:limit] if early_rollout_token_index is not None else None
            
            batch_size = sample_ids.size(0)
            if batch_size == 0:
                return

            # 扩展维度以适配接口: [K, batch, len]
            # 在这里我们将 K 视为 num_iterations 维度传入
            sample_ids_exp = sample_ids.unsqueeze(0).expand(K, -1, -1)
            
            # 生成 K 个不同的随机种子
            profiling_seeds = torch.randint(0, 2**32, (K,), device=sample_ids.device)
            
            with torch.no_grad():
                # 调用你现有的支持 semi-offline 的 ELBO 计算函数
                # 注意：这里传入了 sample_gen_mask，这会传递给 forward_process
                # 从而保证 K 次采样都不会 mask 掉固定的 token
                per_token_logps_k = self._get_per_token_logps_for_semi_and_early(
                    self.model, 
                    sample_ids_exp, 
                    logits_to_keep, 
                    profiling_seeds, 
                    generation_mask=sample_gen_mask, 
                    early_rollout_token_index=sample_early_idx, 
                    completion_mask=sample_comp_mask
                )
            
            # --- 统计计算 ---
            # per_token_logps_k shape: [K, batch, logits_to_keep]
            # 计算序列级 ELBO (Average Per-Token ELBO)
            token_elbo = per_token_logps_k * sample_comp_mask.unsqueeze(0)
            seq_len_counts = sample_comp_mask.sum(dim=1).unsqueeze(0)
            seq_elbo = token_elbo.sum(dim=2) / seq_len_counts # [K, batch]
            
            elbo_mean = seq_elbo.mean(dim=0) # [batch]
            elbo_var = seq_elbo.var(dim=0)   # [batch]
            
            # --- 写入文件 ---
            output_file = os.path.join(self.args.output_dir, "semi_offline_variance.jsonl")
            with open(output_file, "a", encoding="utf-8") as f:
                for i in range(batch_size):
                    # 计算 mask 比例，用于辅助分析
                    total_tokens = sample_comp_mask[i].sum().item()
                    fixed_tokens = 0
                    if sample_gen_mask is not None:
                        # generation_mask 中 0 (False) 代表被固定，1 (True) 代表可被 Mask
                        # 只有在 completion 范围内的才算
                        valid_gen_mask = sample_gen_mask[i, -logits_to_keep:]
                        valid_comp_mask = sample_comp_mask[i]
                        # fixed_count = (valid_comp_mask==1) & (valid_gen_mask==0)
                        fixed_tokens = ((valid_comp_mask.bool()) & (~valid_gen_mask.bool())).sum().item()
                    
                    fixed_ratio = fixed_tokens / total_tokens if total_tokens > 0 else 0

                    record = {
                        "step": self.state.global_step,
                        "mode": "semi_offline" if self.args.semi_offline_flag else "standard",
                        "fixed_ratio": fixed_ratio, # 这是一个很重要的指标，证明确实fix了token
                        "elbo_variance": elbo_var[i].item(),
                        "elbo_mean": elbo_mean[i].item(),
                        # "prompt": prompts_text[i],
                        # "completion": completions_text[i][:100] + "..." # 截断以防过长
                    }
