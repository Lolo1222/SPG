...
SEMI_OFFLINE_DATA_PATH="${SEMI_OFFLINE_DATA_PATH:-${REPO_ROOT}/dataset/llada_math_generations_7500_converted.jsonl}"
DATASET="math"
RUN_NAME="${RUN_NAME:-${DATASET}_diffu_grpo_semi0.95_mask_answer_low_confidence_early0.95}"
TRAINER="diffu_grpo"
FORWARD_TYPE="random"
...
if [ ! -f "$SEMI_OFFLINE_DATA_PATH" ]; then
  echo "ERROR: semi-offline dataset not found: $SEMI_OFFLINE_DATA_PATH" >&2
  exit 1
fi
...
ACCEL_CMD=(accelerate launch
  ...
  --semi_offline_masking_batch_size "$SEMI_OFFLINE_MASKING_BATCH_SIZE"
